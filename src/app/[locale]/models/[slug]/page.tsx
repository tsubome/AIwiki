    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {model.developer && (
            <div>
              <dt className="text-sm font-medium text-gray-500">{t('developer')}</dt>
              <dd className="text-gray-900">{model.developer}</dd>
            </div>
          )}
          {model.parameters && (
            <div>
              <dt className="text-sm font-medium text-gray-500">{t('parameters')}</dt>
              <dd className="text-gray-900">{model.parameters}</dd>
            </div>
          )}
          {model.license && (
            <div>
              <dt className="text-sm font-medium text-gray-500">{t('license')}</dt>
              <dd className="text-gray-900">{model.license}</dd>
            </div>
          )}
          {model.releaseDate && (
            <div>
              <dt className="text-sm font-medium text-gray-500">{t('releaseDate')}</dt>
      ...
      {/* GGUF Files Section */}
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h2 className="text-xl font-bold text-gray-900 mb-4">{t('ggufFiles')}</h2>

        {model.ggufFiles.length === 0 ? (
          <p className="text-gray-500">{t('noGgufFiles')}</p>
        ) : (
      ...
      {(model.parent || model.children.length > 0) && (
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">関連モデル</h2>

          {model.parent && (
            <div className="mb-4">
              <h3 className="text-sm font-medium text-gray-500 mb-2">{t('parentModel')}</h3>
              <Link
          ...
          {model.children.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-gray-500 mb-2">{t('childModels')}</h3>
      ...
      {familyModels.length > 1 && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">{t('familyTree')}</h2>
          <p className="text-sm text-gray-500 mb-4">
            {t('currentModel')}: <span className="font-medium text-red-600">{model.name}</span>
          </p>
